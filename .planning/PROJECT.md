...
- **Routes (post-17.2 IA):** `/`, `/about`, `/projects` + `/projects/[slug]`, `/writing` + `/blog/[slug]` + `/blog/feed.xml`, `/uses`, `/prometheus`. Redirects (301): `/events`→`/`, `/photos`→`/`, `/links`→`/about`, `/newsletter`→`/writing`, `/watching`→`/uses`, `/blog`→`/writing`. Removed (404): `/specimen`, `/v3-specimen`.
...
---
*Last updated: 2026-07-02 — v3.0 Phase 17.2 (site information architecture) complete; lean 4-item nav, 5 cut routes 301-redirected + 2 hard-deleted, `/uses` reframed "Things I Love" (folds `/watching`), `/writing` merged with Monty Monthly (folds `/newsletter`), sitemap trimmed to 6 routes with SEO gate updated in lockstep, zero broken internal links. Verification 10/10, review clean. Next: Phase 17.3 Portfolio surface.*
