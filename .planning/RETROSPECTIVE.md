...
- **Rapid sequential `vercel deploy --prod` calls don't always re-alias atomically** — a fourth promote (hero-cycling fix) showed "Aliased https://montysinger.com" in stdout but `montysinger.com` was still serving the previous deploy's content for several minutes. Forcing it with `vercel alias set <deploy-url> montysinger.com` flipped immediately. Lesson: after each promote during fast iteration, verify the alias with `vercel ls --prod` (most-recent should be aliased) and explicitly `vercel alias set` if it didn't. Cheap insurance against silent stale-deploy serving.
