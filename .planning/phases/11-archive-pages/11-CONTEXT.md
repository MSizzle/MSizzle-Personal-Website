...
- **D-10:** **Sticky behavior** — `position: sticky` on the year column with `top: <some-offset>` so the year stays visible during scroll. The Lenis smooth-scroll integration from Phase 8 D-12 cooperates with native CSS sticky positioning. Desktop only — mobile gets a non-sticky heading.

### NEW Data Module: `src/lib/photos.ts`
- **D-11:** **NEW file `src/lib/photos.ts`** exports a typed array of the 6 existing photos with year + filename + alt:
  ```ts
  ...
  export const PHOTOS_BY_YEAR: ArchivePhoto[] = [
    { filename: "000092530012.jpeg", year: 2025, alt: "...", caption: "Film, 2025" },
    { filename: "20230928 MSB_0114.jpg", year: 2023, alt: "...", caption: "..." },
    { filename: "IMG_0028.jpeg", year: 2024, alt: "...", caption: "..." },
    { filename: "IMG_1075.JPG", year: 2024, alt: "...", caption: "..." },
    { filename: "IMG_2129.jpeg", year: 2024, alt: "...", caption: "..." },
    { filename: "Patricof09.jpg", year: 2024, alt: "...", caption: "..." },
  ];
  ```
  Year mappings are educated guesses based on filename prefixes (`20230928 MSB` → 2023; `000092530012` is a film roll number that suggests 2025 per the handoff's PHOTOS[0] caption "A year in motion · 2025–26"). Plan 11-02 confirms via inspecting `git log` on the file or asking the user — if uncertain, default ALL photos to 2025 for now and surface for Monty to correct.
...
- **D-15:** **Email-subscribe footer** — Reuses the existing `/newsletter` integration. Specifically:
  - Wraps the existing newsletter signup form/component (do NOT introduce a second pipeline per ROADMAP Phase 11 risks)
  - Visual style: inverted ink block per handoff. Padding `py-16 px-40` desktop / `py-12 px-7` mobile.
  - Heading: "Receive new essays the morning they're published." in 28px (`text-section-feature` token).
  - Email input + submit button styled in 11px tracked uppercase (`text-meta`-ish).
  - If the existing newsletter component is a full page wrapper that can't be embedded, inline a minimal form here that POSTs to the same backend endpoint. Plan 11-03 inspects `src/app/newsletter/page.tsx` to determine the cleanest approach.
...
- **D-20:** **Past events** use the same `<ListRow>` (PRIM-04 from Phase 9), denser variant: 4-column grid via wrapping div. The non-big ListRow variant is the right call — it's 20px row padding by default which matches handoff.
...
- **D-25:** **Editorial header** appears on `/` (Phase 10), `/writing`, `/events`, `/photos` — same 5-link nav. To avoid duplication:
  - **Option A:** Extract `<EditorialHeader>` to `src/components/home-v2/editorial-header.tsx` (or `src/components/editorial/header.tsx`). Each archive page imports + renders it.
  - **Option B:** Move into `src/app/layout.tsx` and gate via `usePathname()` for the v2.0 routes (`/`, `/writing`, `/events`, `/photos`).
  - **Recommended: Option A.** Extract `<EditorialHeader>` in Plan 11-01 or 11-03. Phase 12's sub-page restyle sweep can later move it to the layout if all sub-pages adopt the editorial header.
...
- **D-33:** **5 plans, 2 waves:**
  ...
  - **Wave 2 (3 plans, can parallelize since each creates/edits a different route):**
    ...
    - **11-04-events-archive** — `src/app/events/page.tsx` (REWRITE). /events layout with giant numerals + Past dense rows. Updates the v1.0 nav gate (if not already extended by 11-03 — first plan to ship in Wave 2 owns this).
