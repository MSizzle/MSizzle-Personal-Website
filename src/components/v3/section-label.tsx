/* ── v3 SectionLabel ──
   site.css lines 51-52:
     .slabel { font-family:var(--font-mono); font-size:var(--text-sm); text-transform:uppercase;
               letter-spacing:0.12em; color:var(--color-text-dim); display:block; margin-bottom:var(--space-8) }
...
export function SectionLabel({ children, numeral }: Props) {
  return (
    <div className="flex items-baseline justify-between gap-4">
      <span className="font-mono text-sm uppercase tracking-[0.12em] text-text-dim block mb-8">
        {children}
      </span>
      {numeral && (
